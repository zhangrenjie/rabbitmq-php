...
try {
    ...
    $arguments->set('x-expires', 30000);//设置队列中消息的过期时间，单位是毫秒

    //通道绑定队列
    $queueName = 'message_queue_ttl';
    $passive = false;
    $durable = true;//是否持久化队列，服务重启队列不会丢失
    $exclusive = false;//是否独占队列，独占队列只允许当前当前连接使用
    $autoDelete = true;//队列消费完是否自动删除（true当消费者断开服务连接后才会自动删除）
    $aoWait = false;
    $channel->queue_declare($queueName, $passive, $durable, $exclusive, $autoDelete, $aoWait, $arguments);
